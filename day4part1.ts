...
const equalArr = (arr1: Array<number>, arr2: Array<number>) => {
  let arr3 = arr1.concat(arr2)
  const arr3unique = [...new Set(arr3)]
  if (arr3unique.length > arr1.length && arr3unique.length > arr2.length) {
    return false
  }
  return true
}
...
for (let i = 0; i < dataArr.length; i++) {
  ...
  const isContained = (equalArr(ranges[0], ranges[1]))
  if (isContained) {
    amount += 1
  }
}
